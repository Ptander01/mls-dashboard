/**
 * seasonDataLoader.ts — Dynamic Season Data Loader
 *
 * Loads 2026 season data from the static JSON file (fetched by the Python pipeline)
 * and provides season-aware data access. The 2025 data remains in the static mlsData.ts
 * module; this loader handles 2026 data on demand.
 */

import type { Player, Match, TeamBudget } from "./mlsData";
import {
  TEAMS,
  PLAYERS as PLAYERS_2025,
  MATCHES as MATCHES_2025,
  TEAM_BUDGETS as TEAM_BUDGETS_2025,
  TOTAL_WEEKS as TOTAL_WEEKS_2025,
} from "./mlsData";

export type SeasonYear = 2025 | 2026;

export interface SeasonData {
  matches: Match[];
  players: Player[];
  teamBudgets: Record<string, TeamBudget>;
  totalWeeks: number;
  seasonYear: SeasonYear;
}

// ─── 2025 static data (always available) ───
const SEASON_2025: SeasonData = {
  matches: MATCHES_2025,
  players: PLAYERS_2025,
  teamBudgets: TEAM_BUDGETS_2025,
  totalWeeks: TOTAL_WEEKS_2025,
  seasonYear: 2025,
};

// ─── 2026 data (loaded on demand) ───
let _season2026: SeasonData | null = null;
let _loadPromise: Promise<SeasonData> | null = null;

/**
 * Load the 2026 season data from the static JSON file.
 * Returns cached data if already loaded.
 */
export async function load2026Data(): Promise<SeasonData> {
  if (_season2026) return _season2026;
  if (_loadPromise) return _loadPromise;

  _loadPromise = fetch("/data/mls2026.json")
    .then((res) => {
      if (!res.ok) throw new Error(`Failed to load 2026 data: ${res.status}`);
      return res.json();
    })
    .then((json) => {
      // Validate and cast the JSON to our types
      const matches: Match[] = (json.matches || []).map((m: any) => ({
        id: m.id,
        week: m.week,
        date: m.date,
        homeTeam: m.homeTeam,
        awayTeam: m.awayTeam,
        homeGoals: m.homeGoals,
        awayGoals: m.awayGoals,
        attendance: m.attendance || 0,
        venue: m.venue || "",
      }));

      const players: Player[] = (json.players || []).map((p: any) => ({
        id: p.id,
        name: p.name,
        team: p.team,
        position: p.position as Player["position"],
        nationality: p.nationality || "Unknown",
        age: p.age || 25,
        games: p.games || 0,
        starts: p.starts || 0,
        minutes: p.minutes || 0,
        goals: p.goals || 0,
        assists: p.assists || 0,
        yellowCards: p.yellowCards || 0,
        redCards: p.redCards || 0,
        shots: p.shots || 0,
        shotsOnTarget: p.shotsOnTarget || 0,
        shotAccuracy: p.shotAccuracy || 0,
        fouls: p.fouls || 0,
        fouled: p.fouled || 0,
        tackles: p.tackles || 0,
        interceptions: p.interceptions || 0,
        crosses: p.crosses || 0,
        offsides: p.offsides || 0,
        salary: p.salary || 0,
      }));

      // Use 2025 budgets as fallback for 2026 (salary data not yet published)
      const teamBudgets: Record<string, TeamBudget> =
        json.teamBudgets && Object.keys(json.teamBudgets).length > 0
          ? json.teamBudgets
          : TEAM_BUDGETS_2025;

      const totalWeeks: number = json.totalWeeks || 5;

      _season2026 = {
        matches,
        players,
        teamBudgets,
        totalWeeks,
        seasonYear: 2026,
      };

      return _season2026;
    })
    .catch((err) => {
      console.error("Failed to load 2026 season data:", err);
      _loadPromise = null;
      throw err;
    });

  return _loadPromise;
}

/**
 * Get season data synchronously. Returns null for 2026 if not yet loaded.
 */
export function getSeasonDataSync(season: SeasonYear): SeasonData | null {
  if (season === 2025) return SEASON_2025;
  return _season2026;
}

/**
 * Get season data, loading if necessary.
 */
export async function getSeasonData(season: SeasonYear): Promise<SeasonData> {
  if (season === 2025) return SEASON_2025;
  return load2026Data();
}

/**
 * Check if 2026 data is already loaded.
 */
export function is2026Loaded(): boolean {
  return _season2026 !== null;
}

/**
 * Get the 2025 static data (always available).
 */
export function get2025Data(): SeasonData {
  return SEASON_2025;
}

// Re-export TEAMS since they're shared across seasons
export { TEAMS };
