...
function triangulatePoly(rings: number[][][]): {
  vertices: number[];
  indices: number[];
} {
...
function createScene(container: HTMLDivElement): SceneCtx {
  ...
  const toMut = (ring: readonly (readonly number[])[]) =>
    ring.map(pt => [...pt]) as number[][];
  ...
  metros.forEach(([lng, lat, intensity]) => {
    ...
    grad.addColorStop(0, `rgba(255, 210, 130, ${0.1 * intensity})`);
...
function TooltipOverlay({
  ...
  return (
    ...
    >
      <div
        className="glass-sm p-3 min-w-[200px]"
...
export default function TravelMap() {
  ...
  const longestTrip =
    allArcs.length > 0 ? Math.max(...allArcs.map(a => a.distance)) : 0;
  ...
  const dbHeadline = useMemo(
    () => dumbbellHeadline(resilienceMetrics),
    [resilienceMetrics]
  );
  const riHeadline = useMemo(
    () => resilienceHeadline(resilienceMetrics),
    [resilienceMetrics]
  );
    ...
    (container: HTMLDivElement) => {
      if (ctxRef.current) {
        ...
        if (ctxRef.current.renderer.domElement.parentNode) {
          ctxRef.current.renderer.domElement.parentNode.removeChild(
            ctxRef.current.renderer.domElement
          );
        }
      }
          ...
          group.children.forEach(child => {
            if (child instanceof THREE.Mesh && child.userData?.isPulse) {
              ...
              (child.material as THREE.MeshBasicMaterial).opacity =
                0.06 * (1 - Math.sin(time * 1.2) * 0.3);
            }
          });
        });

        // Dust drift
        const dustObj = ctx.scene.children.find(
          c => c instanceof THREE.Points && (c as THREE.Points).userData?.isDust
    ...
    <StaggerContainer className="space-y-6 mt-4">
      ...
      <StaggerItem>
        <NeuCard variant="raised" animate={false} className="p-5">
          <p
            ...
            style={{ fontFamily: "Space Grotesk, sans-serif" }}
          >
            <span className="font-semibold text-foreground">Travel Map</span> —
            Visualize the travel burden across MLS clubs. The interactive map
            shows each team's location and travel routes. Select a team to see
            their away schedule, total miles traveled, and longest trips.
            Conference-heavy schedules mean Western teams often face
            significantly more travel than Eastern counterparts.
          </p>
        </NeuCard>
      </StaggerItem>
        ...
        <div>
          <div className="flex items-center gap-2 mb-3 px-1">
            ...
            <span className="text-[10px] text-muted-foreground/60 ml-1">
              Aggregate travel distances across all teams
            </span>
          </div>
          ...
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-[10px] text-muted-foreground mt-0.5">
                Interactive 3D map — drag to orbit, scroll to zoom, click a team
          ...
          <div className="flex justify-center gap-6 mt-3 text-[10px] text-muted-foreground/50">
            ...
            <div className="flex items-center gap-1.5">
              <span
                ...
                style={{
                  background:
                    "radial-gradient(ellipse at 30% 25%, rgba(255,255,255,0.4), #4488aa 50%, rgba(0,0,0,0.5))",
                }}
              />
              <span>Stadium</span>
            </div>
        ...
        <NeuCard delay={0.25} className="p-4">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => {
                  setCurrentWeek(Math.min(33, currentWeek + 1));
                  setIsPlaying(false);
                }}
                className="neu-raised p-1.5 rounded-lg hover:text-cyan transition-colors"
              >
          ...
          <div className="mt-3 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2">
            {weekMatches.slice(0, 10).map(m => {
              ...
              const dist =
                home && away
                  ? calculateDistance(away.lat, away.lng, home.lat, home.lng)
                  : 0;
      ...
      <StaggerItem>
        <div className="mt-6 mb-2">
          ...
          <p className="text-[10px] text-muted-foreground mt-0.5">
            How does travel burden affect on-field results? Explore home/away
            splits and composite resilience scores.
          </p>
        </div>
      </StaggerItem>
        ...
        <div className="space-y-4">
          ...
          <NeuCard className="p-5">
            <div className="flex items-center justify-between mb-1">
              ...
              <CardInsightToggle
                isOpen={showDumbbellInsights}
                onToggle={() => setShowDumbbellInsights(v => !v)}
                isDark={isDark}
                compact
              />
            </div>
          ...
          <NeuCard className="p-5">
            <div className="flex items-center justify-between mb-1">
              ...
              <CardInsightToggle
                isOpen={showResilienceInsights}
                onToggle={() => setShowResilienceInsights(v => !v)}
                isDark={isDark}
                compact
              />
            </div>
      ...
      {/* Maximize Modal */}
      <ChartModal
        isOpen={maximized === "map"}
        onClose={() => setMaximized(null)}
        title={`League Travel Map — Matchweek ${currentWeek}`}
      >
