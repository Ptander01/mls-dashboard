...
export default function TeamBudget() {
  ...
  const COLORS = ['#00d4ff', '#ffb347', '#00c897', '#ff6b6b', '#a78bfa'];
    ...
    <div className="space-y-4 mt-4">
      ...
      <NeuCard delay={0.15} className="p-4">
        <h3 className="text-sm font-semibold mb-3" style={{ fontFamily: 'Space Grotesk' }}>
          Team Budget Breakdown ($ Millions)
        </h3>
        ...
        <div className="flex justify-center gap-6 mt-2">
          {[
            { label: 'Designated Players', color: '#00d4ff' },
            { label: 'TAM', color: '#ffb347' },
            { label: 'GAM', color: '#00c897' },
          ].map(l => (
            ...
            {filteredTeams.slice(0, 15).map(t => (
              <button
                ...
                className={`text-[10px] px-2 py-1 rounded transition-all ${
                  selectedTeam === t.id ? 'neu-pressed text-cyan' : 'neu-raised text-muted-foreground hover:text-foreground'
                }`}
              >
                {t.shortName}
              </button>
