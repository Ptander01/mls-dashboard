...
export default function PitchMatch() {
  ...
  const teamShots = useMemo(() =>
    shotData.filter(s => s.teamId === selectedTeam),
    [selectedTeam]
  );
  ...
  const passNetwork = useMemo(() =>
    passingNetworks.find(pn => pn.teamId === selectedTeam),
    [selectedTeam]
  );

  // Draw heatmap on canvas
  useEffect(() => {
    if (view !== 'heatmap' || !playerHeatmap || !canvasRef.current) return;
        ...
        <NeuCard delay={0.05} className="p-4 lg:col-span-1">
          ...
          <div className="space-y-0.5 max-h-32 overflow-y-auto mb-3">
            {filteredTeams.map(t => (
              <button
                key={t.id}
                onClick={() => { setSelectedTeam(t.id); setSelectedPlayer(null); }}
        ...
        <NeuCard delay={0.12} className="p-4 lg:col-span-3">
          ...
          <div className="relative rounded-xl overflow-hidden" style={{ aspectRatio: '16/10' }}>
            <div
              ...
              style={{ backgroundImage: `url(${PITCH_BG})`, filter: 'brightness(0.6)' }}
            />
            ...
            {view === 'shotmap' && (
              <svg viewBox="0 0 100 65" className="absolute inset-0 w-full h-full" style={{ zIndex: 2 }}>
                {playerShots.slice(0, 200).map((shot, i) => {
                  const color = shot.result === 'goal' ? '#00c897' :
                    shot.result === 'saved' ? '#ffb347' :
                    shot.result === 'off_target' ? '#ff6b6b' : '#666';
                  ...
                  return (
                    <g key={i}>
                      ...
                      <circle
                        ...
                        cy={node.y * 0.65}
                        r={size}
                        fill={getTeam(selectedTeam)?.primaryColor || '#00d4ff'}
                        stroke="#fff"
                        strokeWidth={0.3}
                      />
                      <text
                        x={node.x}
                        y={node.y * 0.65 - size - 1.5}
                        textAnchor="middle"
                        fill="#e8e8f0"
                        fontSize={2.5}
