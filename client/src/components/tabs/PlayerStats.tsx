...
import {
  ...
  computeOutliers,
  scatterCardInsights,
  topScorersCardInsights,
  playerRadarCardInsights,
  playerTableCardInsights,
...
export default function PlayerStats() {
  ...
  const avgGoals =
    filteredPlayers.length > 0
      ? filteredPlayers.reduce((s, p) => s + p.goals, 0) /
        filteredPlayers.length
      : 0;
  const avgAssists =
    filteredPlayers.length > 0
      ? filteredPlayers.reduce((s, p) => s + p.assists, 0) /
        filteredPlayers.length
      : 0;
  const totalGoals = filteredPlayers.reduce((s, p) => s + p.goals, 0);
  const avgShotAcc =
    filteredPlayers.filter(p => p.shotAccuracy > 0).length > 0
      ? filteredPlayers
          .filter(p => p.shotAccuracy > 0)
          .reduce((s, p) => s + p.shotAccuracy, 0) /
        filteredPlayers.filter(p => p.shotAccuracy > 0).length
      : 0;

  const selPlayer = selectedPlayer
    ? filteredPlayers.find(p => p.id === selectedPlayer)
    : null;
  const radarData = selPlayer
    ? [
        ...
        {
          stat: "Assists",
          value: Math.min(100, (selPlayer.assists / 15) * 100),
        },
        { stat: "Shot Acc", value: selPlayer.shotAccuracy },
        {
          stat: "Tackles",
          value: Math.min(100, (selPlayer.tackles / 80) * 100),
        },
        { stat: "Shots", value: Math.min(100, (selPlayer.shots / 80) * 100) },
        {
          stat: "Minutes",
          value: Math.min(100, (selPlayer.minutes / 3000) * 100),
        },
      ]
    : null;
  ...
  const ColorModeToggle = () => (
    <button
      ...
      style={{
        background: "var(--neu-bg-pressed)",
        color:
          colorMode === "position" ? "var(--cyan)" : "var(--muted-foreground)",
        boxShadow:
          colorMode === "position"
            ? isDark
      ...
      title={
        colorMode === "team"
          ? "Switch to position colors"
          : "Switch to team colors"
      }
    >
      <Palette size={11} />
  ...
  return (
    <StaggerContainer className="space-y-6 mt-4">
      ...
      <StaggerItem>
        <NeuCard variant="raised" animate={false} className="p-5">
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            <span className="font-semibold text-foreground">Player Stats</span>{" "}
            — Compare individual player performance across the 2025 MLS season.
            Use the scatter plot to explore relationships between any two
            metrics (e.g., Shots vs Goals). Toggle between team and position
            coloring, and use the trend line to gauge correlation strength.
            Click any player row or dot to view their full performance radar.
          </p>
          <InsightPanel
            insights={insights}
            isDark={isDark}
        ...
        <div>
          <div className="flex items-center gap-2 mb-3 px-1">
            ...
            <span className="text-[10px] text-muted-foreground/60 ml-1">
              Aggregate stats across all filtered players
            </span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <NeuCard animate={false} className="p-4 lg:col-span-2">
            <ChartHeader
              ...
              description={
                <>
                  Pick any two stats and see how every player in the league
                  stacks up. The{" "}
                  <strong className="text-foreground/80">X-axis</strong> and{" "}
                  <strong className="text-foreground/80">Y-axis</strong> are
                  yours to choose — try Shots vs Goals to find clinical
                  finishers, or Minutes vs Assists to spot creative workhorses.
                  Toggle the trend line to see the overall correlation, and
                  switch between{" "}
                  <strong className="text-foreground/80">team</strong> and{" "}
                  <strong className="text-foreground/80">position</strong>{" "}
                  coloring to reveal different patterns. Outliers are
                  automatically flagged.
                </>
              }
              methods={
                <>
                  Scatter axes are user-selectable from: Goals, Assists, Shots,
                  Shots on Target, Shot Accuracy (%), Tackles, Interceptions,
                  Fouls, Yellow Cards, Minutes, Age, Salary. Trend line: OLS
                  linear regression; R² displayed as a badge. Outliers: points
                  with |residual| &gt; 2× standard deviation of residuals. Color
                  modes: TEAM = muted team primary; POSITION = categorical
                  (FW/MF/DF/GK). All stats are season totals (not per-90). Data:
            ...
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-[10px] text-muted-foreground mt-0.5">
                  Season goal leaders ranked by total goals scored
                </p>
              </div>
              <div className="flex items-center gap-2">
                <CardInsightToggle
                  isOpen={showScorersInsights}
                  onToggle={() => setShowScorersInsights(v => !v)}
                  isDark={isDark}
                  compact
                />
              ...
              {topScorers.map((p, i) => (
                ...
                >
                  ...
                  <span
                    ...
                    style={{ backgroundColor: mutedTeamColor(p.team, isDark) }}
                  />
        ...
        <StaggerItem>
          <NeuCard animate={false} glow="cyan" className="p-4">
            <ChartHeader
              ...
              description={
                <>
                  This radar chart maps{" "}
                  <strong className="text-foreground/80">
                    {selPlayer.name}'s
                  </strong>{" "}
                  season profile across six key dimensions. A wider shape means
                  the player contributes across multiple areas — not just goals,
                  but also creativity, defensive work, and discipline. Compare
                  the shape to other players by clicking different rows in the
                  table above.
                </>
              }
              methods={
                <>
                  Radar axes: Goals, Assists, Shots on Target, Tackles,
                  Interceptions, Shot Accuracy. Each axis normalized to 0–100
                  using percentile rank within the filtered player pool (100 =
                  league leader). Polygon area is visual only — not a composite
                  score. Stats are season totals (not per-90 adjusted). Data:
                  2025 MLS regular season.
                </>
              }
              zone2Analysis={
            ...
            <CardInsightSection
              ...
              insights={
                selPlayer
                  ? playerRadarCardInsights(selPlayer, filteredPlayers)
                  : []
              }
              isDark={isDark}
            />
              ...
              <div style={{ height: 220 }}>
                <ResponsiveContainer>
                  <RadarChart data={radarData}>
                    ...
                    <PolarAngleAxis
                      dataKey="stat"
                      tick={{ fill: "var(--table-header-color)", fontSize: 10 }}
                    />
                    <PolarRadiusAxis
                      tick={false}
                      axisLine={false}
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                {(
                  [
                    {
                      label: "Goals",
                      value: selPlayer.goals,
                      color: "var(--cyan)",
                    },
                    {
                      label: "Assists",
                      value: selPlayer.assists,
                      color: "#ffb347",
                    },
                    {
                      label: "Shots",
                      value: selPlayer.shots,
                      color: "#00c897",
                    },
                    {
                      label: "Shot Acc",
                      value: selPlayer.shotAccuracy,
                      color: "#a78bfa",
                      suffix: "%",
                    },
                    {
                      label: "Tackles",
                      value: selPlayer.tackles,
                      color: "#ff6b6b",
                    },
                    {
                      label: "Fouls",
                      value: selPlayer.fouls,
                      color: "#ff8c42",
                    },
                    {
                      label: "Minutes",
                      value: selPlayer.minutes,
                      color: "var(--table-header-color)",
                    },
                    {
                      label: "Yellow",
                      value: selPlayer.yellowCards,
                      color: "#ffb347",
                    },
                    {
                      label: "Salary",
                      value: selPlayer.salary,
                      color: "#00c897",
                    },
                  ] as {
                    label: string;
                  ...
                  <div
                    key={s.label}
                    className="neu-inset rounded-lg p-2 text-center"
                  >
        ...
        <NeuCard animate={false} className="overflow-hidden">
          ...
          >
            <div className="flex-shrink-0">
              ...
              <p className="text-[10px] text-muted-foreground mt-0.5">
                Full roster table — sort by any column, click a row to view the
                player radar
              </p>
            </div>
      ...
      {/* Maximize Modals */}
      <ChartModal
        isOpen={maximized === "scatter"}
        onClose={() => setMaximized(null)}
        title={`${yOpt.label} vs ${xOpt.label}`}
      >
        ...
        <div className="space-y-2">
          {[...filteredPlayers]
            ...
            .map((p, i) => (
              <div
                key={p.id}
                className="flex items-center gap-3 text-sm py-2 px-3 rounded-lg hover:bg-accent transition-colors"
              >
                ...
                <span className="font-mono text-cyan font-bold text-lg">
                  {p.goals}
                </span>
                <div className="w-32 h-2 rounded-full bg-accent overflow-hidden">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-cyan to-cyan/40"
                    style={{
                      width: `${(p.goals / (filteredPlayers[0]?.goals || 1)) * 100}%`,
                    }}
                  />
                </div>
              </div>
      ...
      <ChartModal
        isOpen={maximized === "radar"}
        onClose={() => setMaximized(null)}
        title={
          selPlayer
            ? `${selPlayer.name} \u2014 Performance Radar`
            : "Player Radar"
        }
      >
        {selPlayer && radarData && (
          ...
          >
            <ResponsiveContainer>
              <RadarChart data={radarData}>
                ...
                <PolarAngleAxis
                  dataKey="stat"
                  tick={{ fill: "var(--table-header-color)", fontSize: 14 }}
                />
                <PolarRadiusAxis
                  tick={false}
                  axisLine={false}
...
function ScatterChartWithTrend({
  ...
  const trendLinePoints = useMemo(() => {
    ...
    return {
      xMin,
      xMax,
      yMin: regression.slope * xMin + regression.intercept,
      yMax: regression.slope * xMax + regression.intercept,
    };
  }, [showTrendLine, scatterData, regression]);
        ...
        <ScatterChart margin={{ top: 10, right: 10, bottom: 20, left: 10 }}>
          ...
          {showTrendLine && trendLinePoints && (
            <ReferenceLine
              ...
              stroke={
                isDark ? "rgba(255,179,71,0.55)" : "rgba(180,100,20,0.45)"
              }
              strokeWidth={2}
              strokeDasharray="8 4"
