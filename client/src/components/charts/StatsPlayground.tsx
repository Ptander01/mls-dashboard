...
const RATE_STATS: RateStatDef[] = [
  ...
  { key: "salary", label: "Salary", shortLabel: "Sal", compute: p => p.salary },
  {
    ...
    label: "Shot Accuracy",
    shortLabel: "Sh%",
    compute: p => p.shotAccuracy,
  },
  {
    key: "goalConversion",
    label: "Goal Conversion",
    shortLabel: "GC%",
    compute: p => (p.shots > 0 ? (p.goals / p.shots) * 100 : 0),
  },
  {
    key: "goals90",
    label: "Goals / 90",
    shortLabel: "G/90",
    compute: p => (p.minutes > 0 ? (p.goals / p.minutes) * 90 : 0),
  },
  {
    key: "assists90",
    label: "Assists / 90",
    shortLabel: "A/90",
    compute: p => (p.minutes > 0 ? (p.assists / p.minutes) * 90 : 0),
  },
  {
    key: "shots90",
    label: "Shots / 90",
    shortLabel: "Sh/90",
    compute: p => (p.minutes > 0 ? (p.shots / p.minutes) * 90 : 0),
  },
  {
    key: "tackles90",
    label: "Tackles / 90",
    shortLabel: "Tkl/90",
    compute: p => (p.minutes > 0 ? (p.tackles / p.minutes) * 90 : 0),
  },
  {
    key: "interceptions90",
    label: "Interceptions / 90",
    shortLabel: "Int/90",
    compute: p => (p.minutes > 0 ? (p.interceptions / p.minutes) * 90 : 0),
  },
  {
    key: "fouls90",
    label: "Fouls / 90",
    shortLabel: "Fls/90",
    compute: p => (p.minutes > 0 ? (p.fouls / p.minutes) * 90 : 0),
  },
];
...
function welchTTest(
  a: number[],
  b: number[]
): { t: number; df: number; p: number; meanA: number; meanB: number } {
  const nA = a.length,
    nB = b.length;
  const mA = mean(a),
    mB = mean(b);
...
function CorrelationMatrixWrapper({
  players,
  isDark,
  positionFilter,
  statMode,
  onCellClick,
}: CorrelationMatrixWrapperProps) {
...
function HypothesisTests({ players, isDark }: HypothesisTestProps) {
  ...
  const [groupBy, setGroupBy] = useState<"position" | "age" | "salary">(
    "position"
  );
    ...
    if (groupBy === "position") {
      ...
      for (let i = 0; i < POSITIONS.length; i++) {
        for (let j = i + 1; j < POSITIONS.length; j++) {
          ...
          const statLabel =
            NUMERIC_STATS.find(s => s.key === statKey)?.label || statKey;
          tests.push({
            title: `${a} vs ${b}`,
      ...
      const groups = {
        "Low (<$200K)": players
          .filter(p => p.salary > 0 && p.salary < 200000)
          .map(p => Number(p[statKey]) || 0),
        "Mid ($200K-$1M)": players
          .filter(p => p.salary >= 200000 && p.salary < 1000000)
          .map(p => Number(p[statKey]) || 0),
        "High (>$1M)": players
          .filter(p => p.salary >= 1000000)
          .map(p => Number(p[statKey]) || 0),
      };
      const labels = Object.keys(groups) as (keyof typeof groups)[];
      for (let i = 0; i < labels.length; i++) {
  ...
  const statLabel =
    NUMERIC_STATS.find(s => s.key === testStat)?.label || testStat;
              ...
              style={{
                ...
                background:
                  groupBy === g
                    ...
                    : "transparent",
                color:
                  groupBy === g ? "var(--cyan)" : "var(--muted-foreground)",
        ...
        {results.map((test, i) => (
          ...
          >
            ...
            <div className="flex items-center gap-2 my-2">
              ...
              >
                {test.meanA >= 1000
                  ? `${(test.meanA / 1000).toFixed(0)}K`
                  : test.meanA.toFixed(1)}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span
              ...
              >
                {test.meanB >= 1000
                  ? `${(test.meanB / 1000).toFixed(0)}K`
                  : test.meanB.toFixed(1)}
              </span>
            </div>
...
function DistributionViewer({ players, isDark }: DistributionViewerProps) {
  ...
  const data = useMemo(() => {
    ...
    const values = filtered.map(
      p => Number(p[selectedStat as keyof Player]) || 0
    );
    if (values.length < 5)
      return {
        bins: [],
        stats: { mean: 0, median: 0, std: 0, min: 0, max: 0, n: 0 },
      };
    ...
    const binCount = Math.min(
      25,
      Math.max(8, Math.ceil(Math.sqrt(values.length)))
    );
    ...
    return {
      bins,
      stats: { mean: m, median: med, std, min, max, n: values.length },
    };
  }, [players, selectedStat, posFilter]);

  const maxCount = Math.max(...data.bins.map(b => b.count), 1);
  const statLabel =
    NUMERIC_STATS.find(s => s.key === selectedStat)?.label || selectedStat;
              ...
              style={{
                ...
                background:
                  posFilter === pos
                    ...
                    : "transparent",
                color:
                  posFilter === pos ? "var(--cyan)" : "var(--muted-foreground)",
              }}
            >
              {pos}
            </button>
      ...
      <div className="relative" style={{ height: 180 }}>
        ...
        <div className="flex items-end gap-[1px] h-full pl-4 pr-1">
          {data.bins.map((bin, i) => {
            ...
            const isMeanBin =
              Math.abs(bin.x - data.stats.mean) <
              (data.bins[1]?.x - data.bins[0]?.x || 1);
            return (
              <motion.div
                key={i}
                initial={{ height: 0 }}
...
const StatsPlayground = memo(
  StatsPlaygroundInner,
  (prev, next) => prev.players === next.players
);
